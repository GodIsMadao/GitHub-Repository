...
import com.jzfy.jfchangesystem.adapter.ListViewAdapter1;
...
public class PrinterConnectDialog extends Activity {
	...
	private ListViewAdapter1 mListViewAdapter1 = null;
	...
	private void initView() {
		...
		mListViewAdapter1 = new ListViewAdapter1(this, mList, mHandler);
		list.setAdapter(mListViewAdapter1);
		...
		public void onReceive(Context context, Intent intent) {
			if (GpCom.ACTION_CONNECT_STATUS.equals(intent.getAction())) {
				...
				if (type == GpDevice.STATE_CONNECTING) {
					...
					SetLinkButtonEnable(ListViewAdapter1.DISABLE);
					mPortParam[id].setPortOpenState(false);
					Map<String, Object> map;
					map = mList.get(id);
					map.put(ListViewAdapter1.STATUS,
					...
					SetLinkButtonEnable(ListViewAdapter1.ENABLE);
					mPortParam[id].setPortOpenState(false);
					Map<String, Object> map;
					map = mList.get(id);
					map.put(ListViewAdapter1.STATUS, getString(R.string.connect));
					mList.set(id, map);
					mListViewAdapter1.notifyDataSetChanged();
					...
					Map<String, Object> map;
					map = mList.get(id);
					map.put(ListViewAdapter1.STATUS, getString(R.string.cut));
					mList.set(id, map);
					mListViewAdapter1.notifyDataSetChanged();
	...
	void SetPortParamToView(PortParameters Param) {
		...
		map.put(ListViewAdapter1.INFO, info);
	...
	void SetLinkButtonEnable(String s) {
		...
		for (int i = 0; i < GpPrintService.MAX_PRINTER_CNT; i++) {
			map = mList.get(i);
			map.put(ListViewAdapter1.BT_ENABLE, s);
			mList.set(i, map);
		}
		mListViewAdapter1.notifyDataSetChanged();
	}
	...
	private List<Map<String, Object>> getOperateItemData() {
		...
		for (int i = 0; i < GpPrintService.MAX_PRINTER_CNT; i++) {
			...
			map.put(ListViewAdapter1.IMG, PrinterImage[i]);
			map.put(ListViewAdapter1.TITEL, getString(PrinterID[i]));
			if (mPortParam[i].getPortOpenState() == false)
				map.put(ListViewAdapter1.STATUS, getString(R.string.connect));
			else
				map.put(ListViewAdapter1.STATUS, getString(R.string.cut));
			String str = getPortParamInfoString(mPortParam[i]);
			map.put(ListViewAdapter1.INFO, str);
			map.put(ListViewAdapter1.BT_ENABLE, "enable");
			list.add(map);
		}
		return list;
	}
	...
	void connectOrDisConnectToDevice(int PrinterId) {
		...
		int rel = 0;
		Log.e(DEBUG_TAG,
				String.valueOf(mPortParam[PrinterId].getPortOpenState()));
		if (mPortParam[PrinterId].getPortOpenState() == false) {
			if (CheckPortParamters(mPortParam[PrinterId])) {
				...
				case PortParameters.ETHERNET:
					try {
						rel = mGpService.openPort(PrinterId,
								mPortParam[PrinterId].getPortType(),
								mPortParam[PrinterId].getIpAddr(),
								mPortParam[PrinterId].getPortNumber());
					} catch (RemoteException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
				...
				case PortParameters.BLUETOOTH:
					try {
						...
						ShareHelper sh=new ShareHelper(PrinterConnectDialog.this);
						sh.save_printaddress(mPortParam[PrinterId].getBluetoothAddr());
					} catch (RemoteException e) {
						e.printStackTrace();
					}
					break;
			...
			SetLinkButtonEnable(ListViewAdapter1.DISABLE);
			Map<String, Object> map = mList.get(PrinterId);
			map.put(ListViewAdapter1.STATUS, getString(R.string.cutting));
			mList.set(PrinterId, map);
			mListViewAdapter1.notifyDataSetChanged();
			try {
	...
	public Handler mHandler = new Handler(new Handler.Callback() {
		...
		public boolean handleMessage(Message message) {
			switch (message.what) {
			case ListViewAdapter1.MESSAGE_CONNECT:
